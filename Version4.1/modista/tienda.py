...
#Import para poder usar las funciones de fechas de python
...
class grafico(models.Model):
	...
	now=date.today()
	anio=fields.Integer(string="Año",default=now.year)
	...
	def confirmar(self):
		if self.fecha != '0':
			...
			self.env.cr.execute('Select sum(coste * cantidad) from modista_gastos where extract(month from fecha_c) =' + self.fecha+' and extract(year from fecha_c)='+str(self.anio))
			...
			self.env.cr.execute('SELECT sum(precio) FROM modista_pedido where pagado = \'1\' and extract(month from fecha_p) ='+ self.fecha+' and extract(year from fecha_p)='+str(self.anio))
